...
class StockBalanceReport:
		...
		for fieldname in ["warehouse", "item_code", "item_group", "warehouse_type"]:
			if self.filters.get(fieldname):
				if fieldname == "warehouse":
					all_warehouses = self.get_warehouses_list()
					query = query.where(table[fieldname].isin(all_warehouses))
				elif fieldname == "item_code":
					items = to_list(self.filters.get(fieldname))
					query = query.where(table[fieldname].isin(items))
				else:
					query = query.where(table[fieldname] == self.filters.get(fieldname))
		return query.run(as_dict=True)

	def prepare_stock_ledger_entries(self):
		frappe.flags.sql_debug = True
	...
	def apply_warehouse_filters(self, query, sle) -> str:
		...
		if self.filters.get("warehouse"):
			query = query.where(sle.warehouse.isin(all_warehouses))
	...
	def get_warehouses_list(self):
		if warehouses := self.filters.get("warehouse"):
			...
			all_warehouses = set()
			for w in warehouses:
				children = get_descendants_of("Warehouse", w, ignore_permissions=True)
				all_warehouses.update(children)
				all_warehouses.add(w)
	...
	def apply_items_filters(self, query, item_table) -> str:
		...
		for field in ["item_code", "brand"]:
			if not self.filters.get(field):
				...
				items = to_list(self.filters.get(field))
				query = query.where(item_table.name.isin(items))
			else:
				brands = to_list(self.filters.get(field))
				query = query.where(item_table[field].isin(brands))
...
def to_list(value):
		...
		if isinstance(value, str):
			# If comma-separated string, split; else single element list
