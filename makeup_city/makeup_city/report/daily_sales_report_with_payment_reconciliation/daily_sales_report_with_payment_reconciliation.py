...
def execute(filters=None):
    ...
    invoices = frappe.db.sql(f"""
        ...
        LEFT JOIN `tabPOS Closing Shift` AS pos_closing ON pos_closing.pos_opening_shift = si.posa_pos_opening_shift
        WHERE {condition_str}
        ORDER BY si.name
    """, filters, as_dict=True)

    # Step 4: Group by invoice
    summary = {}
    for row in invoices:
        key = row.invoice
        ...
        # Assign MOP data to the correct dynamic column slot
        for idx, mop in enumerate(mop_list, start=1):
    ...
    # Step 5: Add difference
    for row in summary.values():
        row["difference"] = flt(row["grand_total"]) - flt(row["total_net_amount"])

    # Step 6: Define columns
    columns = [
        {"label": "Sales Invoice", "fieldname": "invoice", "fieldtype": "Link", "options": "Sales Invoice", "width": 150},
